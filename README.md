...
Generated identifiers default to **Include this ID in JSON**. This checkbox affects only report JSON output; it does not accept, reject, or teach from an annotation. Choose an output format and then **Generate JSON**. Legacy output always uses the three required arrays with escaped quotes and readable separator spacing. Entity-record output groups values by highlighted name and escapes the complete compact JSON array for insertion inside a prompt/completion JSON string.

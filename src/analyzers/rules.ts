...
/** Capitalized/name-like token shared by the broader report-domain patterns. */
const PROPER_TOKEN = String.raw`[A-Z0-9][A-Za-z0-9'’&./-]*`;
/** Proper name of up to ten tokens, allowing common lower-case connectors. */
const PROPER_NAME = String.raw`${PROPER_TOKEN}(?:\s+(?:(?:of|the|and)\s+)?${PROPER_TOKEN}){0,9}`;
/** Shorter proper-name form used after a maritime descriptor. */
const SHORT_PROPER_NAME = String.raw`${PROPER_TOKEN}(?:\s+(?:(?:of|the|and)\s+)?${PROPER_TOKEN}){0,4}`;
/** Maritime descriptors used as high-precision context for names and classes. */
...
export function analyzeWithRules(report: string): CandidateAnnotation[] {
  ...
  // Capture the proper-name portion after a maritime descriptor while
  // excluding generic determiners that would make poor annotations.
  ...
  // Match named facilities and infrastructure, trim leading determiners, and
  // retain exact offsets into the unchanged source report.
  ...
  // Cover constructions such as "Foo Base Bar" that extend beyond the word
  // "Base" and would otherwise be truncated by the suffix-oriented pattern.

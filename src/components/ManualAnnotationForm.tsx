...
/** Exact source-report range selected in the textarea. */
interface Selection {
  start: number;
  end: number;
  text: string;
...
/** Inputs and completion callbacks owned by the report-level controller. */
interface Props {
  report: string;
  selection: Selection;
...
/** Stable display order for the identifier-family checkboxes. */
const ID_TYPES: IdType[] = ["SCONUM", "BE", "BE_OSUFFIX", "SK", "EQPCODE", "CENOT", "ELNOT"];
/** Reviewer-friendly entity labels stored with their internal class values. */
...
/**
 * Collects the semantic class and possible ID families for a missed span.
 * The form delegates final offset/type validation to `createManualAnnotation`
 * and emits only a complete annotation to its parent.
 */
export function ManualAnnotationForm({ report, selection, onAdd, onCancel }: Props) {
  /** Draft values remain local until Add annotation succeeds. */
  ...
  /** Adds or removes one ID family and clears a stale validation message. */
  const toggleType = (type: IdType) => {
    setIdTypes((current) => current.includes(type) ? current.filter((item) => item !== type) : [...current, type]);
    setProblem(undefined);
  };

  /** Validates the draft and returns a normalized annotation to the app. */
  const add = () => {
    if (!idTypes.length) {
